...
static void	ft_create_skin(t_data *data, t_txtr *txtr, t_imgs *skin)
{
	...
	if (txtr->map->coor[txtr->skin->y / H_CASE][txtr->skin->x / W_CASE]
		== '2' && 20 <= coef_x && coef_x <= 60 && 20 <= coef_y && coef_y <= 60)
		txtr->map->coor[txtr->skin->y / H_CASE][txtr->skin->x / W_CASE]
			= '0';
...
int	ft_refresh_image(t_mlx *mlx)
{
	...
	mlx->txtr->map->img->data->img
		= mlx->txtr->map->img->frame[BACKGROUND_SIZE - 1];
