...
static int	ft_create_bkgd(t_data *data,
t_txtr *txtr, unsigned int x, unsigned int y)
{
	if ('0' <= txtr->map->coor[y][x]
		&& txtr->map->coor[y][x] <= '4')
	{
		txtr->map->img->data->img = txtr->map->frame[BKGD_SIZE - 1];
...
static int	ft_create_skin(t_data *data, t_txtr *txtr, t_imgs *skin)
{
	...
	if (ft_create_bkgd(data, txtr, skin->x, skin->y) == -1
...
{
	mlx->txtr->map->img->data->img = mlx->txtr->map->frame[BKGD_SIZE - 1];
