...
static void	ft_create_sprite(t_data *data, t_imgs *sprt, unsigned int x, unsigned int y)
{
	int		tmp;
...
static void	ft_create_move(t_data *data, t_txtr *txtr, unsigned int x, unsigned int y)
{
	mlx_draw_image(data, txtr->map->img->data, x, y);
	if (txtr->map->coor[y][x] == '2')
		ft_create_sprite(data, txtr->clct, x, y);
	else if (txtr->exit->x == x && txtr->exit->y == y)
		ft_create_sprite(data, txtr->exit, x, y);
}

static void	ft_create_skin(t_data *data, t_txtr *txtr, unsigned int x, unsigned int y)
{
	int post;
	...
	{
		if (0 < post && post <= 7)
			ft_create_move(data, txtr, x, y - 1);
		else if (8 < post && post <= 15)
			ft_create_move(data, txtr, x + 1, y);
	...
	while (++y < mlx->txtr->map->height)
	{
		...
		while (++x < mlx->txtr->map->width)
		{
			...
			if (mlx->txtr->map->coor[y][x] == '2')
			{
				...
				ft_create_sprite(mlx->txtr->data, mlx->txtr->clct, x, y);
				mlx->txtr->clct->x++;
			}
		}
	}
	mlx_draw_image(mlx->txtr->data, mlx->txtr->map->img->data, mlx->txtr->exit->x, mlx->txtr->exit->y);
	ft_create_sprite(mlx->txtr->data, mlx->txtr->exit, mlx->txtr->exit->x, mlx->txtr->exit->y);
	mlx_draw_image(mlx->txtr->data, mlx->txtr->map->img->data, mlx->txtr->skin->x, mlx->txtr->skin->y);
	ft_create_skin(mlx->txtr->data, mlx->txtr, mlx->txtr->skin->x, mlx->txtr->skin->y);
